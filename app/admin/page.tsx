...
export default function AdminPage() {
  ...
  useEffect(() => {
    const login = localStorage.getItem("argent_admin_login");
    setIsLoggedIn(login === "true");
    setCheckedLogin(true);
  }, []);

  useEffect(() => {
    if (checkedLogin && isLoggedIn) fetchProducts();
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#f8f5f2] px-5">
        <div className="w-full max-w-md rounded-[2rem] bg-white p-10 text-center shadow-sm">
          ...
          <h1 className="mb-4 text-3xl font-bold">無權限進入 ☁️</h1>
          <p className="mb-6 text-sm leading-8 text-[#6b5c50]">
            請先登入後台。
          </p>
          <a href="/admin-login" className="inline-block rounded-full bg-black px-8 py-4 text-white">
            前往登入
          </a>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-[#f8f5f2] px-5 py-8 text-[#3d3d3d]">
      ...
      <div className="rounded-3xl bg-white p-6 shadow-sm">
        ...
        <div className="space-y-4">
          ...
          <select className="w-full rounded-2xl border p-4" value={category} onChange={(e) => setCategory(e.target.value)}>
          ...
          <input className="w-full rounded-2xl border p-4" placeholder="商品排序，數字越小越前面" type="number" value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} />
            ...
            <div className="rounded-2xl bg-[#f8f5f2] p-4">
              ...
              <div className="grid grid-cols-4 gap-3">
                {editingImages.map((img, index) => (
                  <img key={index} src={img} className="aspect-square rounded-xl object-cover" />
                ))}
              </div>
            </div>
          )}
            ...
            return (
              <div key={product.id} className="rounded-2xl border p-4">
                <div className="mb-3 flex flex-wrap gap-2">
                  <span className={`rounded-full px-3 py-1 text-xs ${active ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-500"}`}>
                    {active ? "上架中" : "已下架"}
                  </span>

                  <span className={`rounded-full px-3 py-1 text-xs ${soldOut ? "bg-red-100 text-red-600" : "bg-[#f3ede6] text-[#8b6f5c]"}`}>
                    {soldOut ? "已售完" : "可下單"}
                  </span>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-sm text-gray-500">NT$ {product.price}</p>
                    <p className="mt-1 text-xs text-[#b58b6b]">{product.category}｜排序 {product.sort_order ?? 0}</p>
                    <p className="mt-1 text-xs text-gray-400">圖片 {productImages.length} 張</p>
                  </div>
                </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  ...
                  <button onClick={() => deleteProduct(product.id)} className="rounded-full bg-red-500 py-3 text-sm text-white">
                    刪除
                  </button>
                </div>

                <div className="mt-3 grid grid-cols-2 gap-3">
                  <button onClick={() => toggleActive(product.id, active)} className="rounded-full border py-3 text-sm">
                    {active ? "下架" : "上架"}
                  </button>

                  <button onClick={() => toggleSoldOut(product.id, soldOut)} className="rounded-full border py-3 text-sm">
                    {soldOut ? "恢復販售" : "設為售完"}
                  </button>
                </div>
              </div>
